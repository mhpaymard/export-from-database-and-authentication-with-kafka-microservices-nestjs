...
export class ProtoService implements OnModuleInit {
  ...
  // Helper method to encode message
  ...
  // Helper method to decode message
  decode<T = any>(messageType: protobuf.Type, buffer: Buffer): T {
    const message = messageType.decode(buffer);
    const decoded = messageType.toObject(message, {
      longs: Number,
      enums: String,
      bytes: String, // Convert bytes to base64 string for JSON serialization
      defaults: true,
      arrays: true,
      objects: true,
      oneofs: true,
